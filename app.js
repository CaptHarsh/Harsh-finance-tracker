...
function quick(v){type.value='Expense';cats();category.value=v;amount.focus();}
cats(); type.onchange=cats;

function rebuildMonths(){
const m=[...new Set(s.tx.map(x=>x.month))];
monthFilter.innerHTML='<option value="">📅 All Months</option>'+m.map(x=>`<option>${x}</option>`).join('');
}

function addTx(){
const a=parseFloat(amount.value); if(!a)return;
if(editId){removeTx(editId,false);}
const d=new Date();
...
const days=Math.max(1,new Date(today.getFullYear(),today.getMonth()+1,0).getDate()-today.getDate()+1);

rbl.textContent='₹'+Math.round(s.rbl);
kotak.textContent='₹'+Math.round(s.kotak);
cc.textContent='₹'+Math.round(s.cc);
net.textContent='₹'+Math.round(nw);
safe.textContent='₹'+Math.round(safe/days);
daily.textContent='₹'+Math.round(safe/days);
weekly.textContent='₹'+Math.round((safe/days)*7);
reimb.textContent='₹'+Math.round(s.reimb);
util.textContent=((s.cc/s.ccLimit)*100).toFixed(1)+'%';
forecast.textContent='₹'+Math.round(nw+s.reimb);
monthName.textContent=today.toLocaleString('default',{month:'short',year:'2-digit'});

const pct=Math.min(100,(nw/s.goal)*100);
...
list.innerHTML=tx.map(t=>`<div class="tx">${t.cat} <b>₹${t.amt}</b><br>${t.date}<br><button class="smallbtn" onclick="editTx(${t.id})">✏️</button> <button class="smallbtn" onclick="removeTx(${t.id})">🗑️</button></div>`).join('');
}
render();